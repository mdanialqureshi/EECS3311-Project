...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			if cmd_name ~ "test" then 
				 if attached {ETF_INT_ARG} args[1] as p_threshold and then 1 <= p_threshold.value and then p_threshold.value <= 101 then 
					 create {ETF_TEST} Result.make ("test", [p_threshold.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
	...
	do
		...
		loop
			...
			if cmd_name ~ "test" then 
				if NOT( ( args.count = 1 ) AND THEN attached {ETF_INT_ARG} args[1] as p_threshold and then 1 <= p_threshold.value and then p_threshold.value <= 101) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"test(p_threshold: THRESHOLD = 1 .. 101)")
				end
